...
opts = {
  wallet_quantity_request_dto: DocspaceApiSdk::WalletQuantityRequestDto.new # WalletQuantityRequestDto | 
}
...
> <Array(<TenantWalletSettingsWrapper>, Integer, Hash)> get_tenant_wallet_settings_with_http_info
...
begin
  ...
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <TenantWalletSettingsWrapper>
rescue DocspaceApiSdk::ApiError => e
...
api_instance = DocspaceApiSdk::Portal::PaymentApi.new
service = DocspaceApiSdk::TenantWalletService::AITools # TenantWalletService | The wallet service type.
...
api_instance = DocspaceApiSdk::Portal::PaymentApi.new
opts = {
  sales_requests_dto: DocspaceApiSdk::SalesRequestsDto.new({email: 'user@example.com', message: 'I would like to inquire about pricing'}) # SalesRequestsDto | 
}
...
> <TenantWalletSettingsWrapper> set_tenant_wallet_settings(opts)
...
> <Array(<TenantWalletSettingsWrapper>, Integer, Hash)> set_tenant_wallet_settings_with_http_info(opts)
...
api_instance = DocspaceApiSdk::Portal::PaymentApi.new
opts = {
  quantity_request_dto: DocspaceApiSdk::QuantityRequestDto.new # QuantityRequestDto | 
}
