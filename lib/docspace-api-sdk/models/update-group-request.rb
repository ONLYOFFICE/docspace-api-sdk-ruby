...
module DocspaceApiSdk
  ...
  class UpdateGroupRequest < ApiModelBase
    # The list of user IDs to add to the group.
    attr_accessor :members_to_add

    # The list of user IDs to remove from the group.
    attr_accessor :members_to_remove
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::UpdateGroupRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::UpdateGroupRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'members_to_add')
        if (value = attributes[:'members_to_add']).is_a?(Array)
          self.members_to_add = value
        end
      end

      if attributes.key?(:'members_to_remove')
        if (value = attributes[:'members_to_remove']).is_a?(Array)
          self.members_to_remove = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          members_to_add == o.members_to_add &&
          members_to_remove == o.members_to_remove &&
    ...
    def hash
      [members_to_add, members_to_remove, group_manager, group_name].hash
    end
