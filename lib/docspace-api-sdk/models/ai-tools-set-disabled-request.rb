...
module DocspaceApiSdk
  class AiToolsSetDisabledRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::AiToolsSetDisabledRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::AiToolsSetDisabledRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tool_names')
        if (value = attributes[:'tool_names']).is_a?(Array)
    ...
    # Custom attribute writer method with validation
    # @param [Object] tool_names Value to be assigned
    def tool_names=(tool_names)
      if tool_names.nil?
        fail ArgumentError, 'tool_names cannot be nil'
      end
    ...
    def hash
      [server_type, tool_names, entity_id].hash
    end
