  ...
  class DocsCloudTenantInfo < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'users_limit' => :'DocsCloudUsersLimit',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::DocsCloudTenantInfo` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::DocsCloudTenantInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'users_limit')
        self.users_limit = attributes[:'users_limit']
      end
    ...
    def hash
      [license, server, users_limit, stats].hash
    end
