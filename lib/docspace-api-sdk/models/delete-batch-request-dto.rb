...
module DocspaceApiSdk
  ...
  class DeleteBatchRequestDto < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'file_ids' => :'Array<DeleteBatchRequestDtoAllOfFileIds>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::DeleteBatchRequestDto` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::DeleteBatchRequestDto`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'immediately')
        self.immediately = attributes[:'immediately']
      end
    end
    ...
    def hash
      [return_single_operation, folder_ids, file_ids, delete_after, immediately].hash
    end
