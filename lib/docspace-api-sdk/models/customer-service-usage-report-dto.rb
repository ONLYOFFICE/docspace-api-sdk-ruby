...
module DocspaceApiSdk
  ...
  class CustomerServiceUsageReportDto < ApiModelBase
    ...
    def self.openapi_types
      {
        :'collection' => :'Array<CustomerServiceUsageDto>',
        :'offset' => :'Integer',
        :'limit' => :'Integer',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::CustomerServiceUsageReportDto` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::CustomerServiceUsageReportDto`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'collection')
        if (value = attributes[:'collection']).is_a?(Array)
    ...
    def hash
      [collection, offset, limit, total_quantity, total_page, current_page].hash
    end
