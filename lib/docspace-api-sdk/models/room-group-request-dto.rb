  ...
  class RoomGroupRequestDto < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::RoomGroupRequestDto` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::RoomGroupRequestDto`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @icon.to_s.length > 50
        invalid_properties.push('invalid value for "icon", the character length must be smaller than or equal to 50.')
      end

      if @icon.to_s.length < 0
        invalid_properties.push('invalid value for "icon", the character length must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @icon.to_s.length > 50
      return false if @icon.to_s.length < 0
    ...
    def icon=(icon)
      ...
      if icon.to_s.length > 50
        fail ArgumentError, 'invalid value for "icon", the character length must be smaller than or equal to 50.'
      end

      if icon.to_s.length < 0
        fail ArgumentError, 'invalid value for "icon", the character length must be greater than or equal to 0.'
      end
