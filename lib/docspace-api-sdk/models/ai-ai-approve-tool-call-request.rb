...
module DocspaceApiSdk
  class AiAiApproveToolCallRequest < ApiModelBase
    attr_accessor :result

    # Persist auto-approve for this tool's name.
    attr_accessor :allow_always
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::AiAiApproveToolCallRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::AiAiApproveToolCallRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'allow_always')
        self.allow_always = attributes[:'allow_always']
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] idx Value to be assigned
    def idx=(idx)
      if idx.nil?
        fail ArgumentError, 'idx cannot be nil'
      end
    ...
    def hash
      [result, allow_always, thread_id, message_id, idx, message, action_args, entity_id, profile_id].hash
    end
