...
module DocspaceApiSdk
  ...
  class ExternalDbSyncTaskDto < ApiModelBase
    # The task ID.
    attr_accessor :id
    ...
    def self.openapi_types
      {
        ...
        :'forms' => :'Array<ExternalDbSyncFormResultDto>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::ExternalDbSyncTaskDto` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::ExternalDbSyncTaskDto`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, error, percentage, is_completed, status, forms].hash
    end
