...
module DocspaceApiSdk
  class AiToolsSetAllowAlwaysRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::AiToolsSetAllowAlwaysRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::AiToolsSetAllowAlwaysRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] tool_name Value to be assigned
    def tool_name=(tool_name)
      if tool_name.nil?
        fail ArgumentError, 'tool_name cannot be nil'
      end
    ...
    def hash
      [server_type, tool_name, value, entity_id].hash
    end
