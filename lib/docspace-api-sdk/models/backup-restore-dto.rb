...
module DocspaceApiSdk
  ...
  class BackupRestoreDto < ApiModelBase
    # The backup ID.
    ...
    def self.openapi_types
      {
        ...
        :'storage_params' => :'Array<ItemKeyValuePairObjectObject>',
        :'notify' => :'Boolean',
        :'dump' => :'Boolean'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DocspaceApiSdk::BackupRestoreDto` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DocspaceApiSdk::BackupRestoreDto`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'storage_params')
        if (value = attributes[:'storage_params']).is_a?(Array)
    ...
    def hash
      [backup_id, storage_type, storage_params, notify, dump].hash
    end
